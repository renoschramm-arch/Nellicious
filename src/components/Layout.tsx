  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <nav className="sm:hidden fixed inset-x-0 bottom-0 z-10 border-t border-border bg-surface/95 backdrop-blur pb-[env(safe-area-inset-bottom)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)]">
        <div className="max-w-3xl mx-auto flex items-center justify-center gap-1 px-2.5 py-1.5">
          {navItems.map((item) => (
            <NavLink
              key={item.to}
              to={item.to}
              ...
              className={({ isActive }) =>
                `flex items-center justify-center px-3.5 py-2 rounded-full text-base font-semibold whitespace-nowrap transition-colors ${
                  isActive
                    ? 'bg-primary text-on-primary'
                    : 'text-text-muted hover:text-text'
                }`
